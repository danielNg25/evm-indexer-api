...
import {UniswapV2LibraryCustomFee} from "./libraries/UniswapV2LibraryCustomFee.sol";
...
contract UniversalRouterCustomFee is IHookConfig, Ownable2Step {
    using BytesLib for bytes;
    using Pool for bytes;
    using UniswapV2LibraryCustomFee for IUniswapV2Pair;

    uint256 public constant DEFAULT_V2_FEE = 30;
    ...
    mapping(address => uint256) public v2FactoryFee;
    mapping(address => uint256) public v2Fee;
    ...
    constructor(
        ...
        for (uint256 i = 0; i < _v2Factories.length; i++) {
            _setV2FactoryFee(_v2Factories[i], _v2Fees[i]);
        }
    }
    ...
    function setApprovedSig(
        ...
        _setApprovedSig(sig, callbackType);
    }

    function setApprovedSigs(
        bytes4[] memory sigs,
        CallbackType[] memory callbackTypes
    ) external onlyOwner {
        for (uint256 i = 0; i < sigs.length; i++) {
            _setApprovedSig(sigs[i], callbackTypes[i]);
        }
    }

    function _setApprovedSig(bytes4 sig, CallbackType callbackType) internal {
    ...
    function exactOutputInternal(
        ...
        try IUniswapV3PoolImmutables(pool).liquidity() {
            ...
            amountIn = IUniswapV2Pair(pool).getAmountIn(
                tokenIn,
                amountOut,
                getV2Fee(pool),
                FEE_DENOMINATOR
    ...
    function getV2Fee(address pool) public returns (uint256) {
        uint256 fee = v2Fee[pool];
        if (fee == 0) {
            try IUniswapV2Pair(pool).factory() returns (address factory) {
                fee = v2FactoryFee[factory];
            } catch {
                fee = v2FactoryFee[address(1)]; // 0x1 is the default factory address
            }
    ...
    function setV2FactoryFee(address factory, uint256 fee) external onlyOwner {
        _setV2FactoryFee(factory, fee);
    }

    function _setV2FactoryFee(address factory, uint256 fee) internal {
        v2FactoryFee[factory] = fee;
    }

    function _setV2Fee(address pool, uint256 fee) internal {
        v2Fee[pool] = fee;
    }
