...
use alloy::primitives::{aliases::I24, uint, Uint, U160, U256};
...
pub trait TickMath: Sized {
    fn get_sqrt_ratio_at_tick(tick: I24) -> Result<Self>;
    fn get_tick_at_sqrt_ratio(self) -> Result<I24>;
}
...
#[inline]
pub fn get_sqrt_ratio_at_tick(tick: I24) -> Result<U160> {
    let abs_tick = tick.abs().as_i32();

    if abs_tick > MAX_TICK.as_i32() {
        return Err(anyhow!("Invalid tick: {}", tick));
    }
...
pub fn get_tick_at_sqrt_ratio<const BITS: usize, const LIMBS: usize>(
    ...
    if (sqrt_ratio_x96 - MIN_SQRT_RATIO) > MAX_SQRT_RATIO_MINUS_MIN_SQRT_RATIO_MINUS_ONE {
        return Err(anyhow!("Invalid sqrt price: {}", sqrt_ratio_x96));
    }
...
mod tests {
    ...
    #[test]
    #[should_panic(expected = "Invalid tick: -887273")]
